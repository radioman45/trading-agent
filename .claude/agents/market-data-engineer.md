...
- **스크립트 먼저.** OHLCV·기술지표·기본 시세는 손으로 찾지 말고 번들 스크립트(`collect_market_data.py`)를 실행해 받는다. 스크립트 산출(`00_indicators.json`의 `fast_info`)은 스냅샷의 주가·시총·발행주식수 1차 후보다 — 웹 출처와 교차 확인 후 채택한다.
...
- 스크립트가 실패(`status: error`)하면 1회 재시도(한국 코드는 `.KS`/`.KQ` 접미사 자동 시도됨). 그래도 실패하면 웹 검색으로 시세만 수집해 스냅샷을 채우고, `data_quality.notes`에 "OHLCV/기술지표 미생성"을 명시한다.
