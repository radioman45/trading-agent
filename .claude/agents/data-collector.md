...
- **사실만 고정한다.** 스냅샷에는 공모가(밴드/확정)·발행신주·상장후 주식수·시총·free float 4필드·락업·지수편입·환율·세그먼트 재무 핵심값만 담는다(신규상장엔 거래 주가·거래 멀티플이 없다). 투자 판단·전망·목표가는 절대 넣지 않는다.
...
- **시점 가드·출처 실재성 [HARD] (2026-06-24 사고 반영):** 아직 일어나지 않은 일(미확정 공모가, 상장 전 거래가, 마감 안 된 세션 종가)을 *확정 사실*로 쓰지 않는다 → `source_status:preliminary`/`unavailable`. 모든 `source`는 이번 실행에서 실제 조회한 EDGAR/DART 문서·URL만(기억·날조 인용 금지). 산식 불일치는 `self_check` FAIL로. 상세 `docs/anti-hallucination-verification-plan.md`.
