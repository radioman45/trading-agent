...
description: 트레이딩 분석용 공통 시장 스냅샷(00_market_snapshot.json)과 일봉 OHLCV·기술지표(00_ohlcv_daily.csv, 00_indicators.json) 수집 방법론. 분석 대상의 시장(한국/미국/크립토)을 감지하고 주가·시가총액·발행주식수(크립토는 유통량)·환율·벤치마크·재무 핵심값·밸류에이션 배수·기술지표를 단일 진실 소스(SSOT)로 고정한다. market-data-engineer 에이전트가 스냅샷을 작성할 때, 또는 "시장 스냅샷", "데이터 고정", "기술지표 수집", "market snapshot", 비트코인·이더리움 등 크립토 시세 고정을 요청받을 때 반드시 이 스킬을 사용한다.
---
...
- **`session` 블록(기계 판정):** market_session(`pre_open|regular|post_close|weekend`, CRYPTO는 `always_open`)·`data_as_of`·`last_close_is_final`·`staleness_calendar_days`·`stale_feed_suspect`·`intraday_data_gap`·`requested_at_utc` 등. 휴장일 캘린더는 내장하지 않으므로(`calendar_status: approx_no_holiday_calendar`) `stale_feed_suspect: true`는 "휴장이었는지 피드 지연인지 확인하라"는 신호다 — 웹 확인 후 `data_quality.notes`에 원인을 남긴다. `intraday_data_gap: true`(장중 세션인데 당일 봉 없음)는 **평일 휴장 또는 피드 미갱신** 신호 — 같은 방식으로 원인을 확인·기록하고, 그날을 "장중"으로 서술하지 않는다. **CRYPTO(24/7)는** 일봉이 UTC 자정 마감이라 당일 UTC 봉은 항상 미확정(`last_close_is_final: false`)이며, weekend·gap 개념이 없다(`calendar_status: continuous_24_7`) — 지연은 stale_feed_suspect가 담당.
- **`cross_check` 블록(이중 소스):** 미국 티커는 stooq → nasdaq 체인, 크립토(USD/USDT 쿼트)는 coinbase → binance(USDT 근사, 통상 괴리 <0.3%) 체인으로 마지막 종가를 같은 거래일에 대조한다(허용 오차 기본 1%). `mismatch`면 값 채택 전 원인 규명 — 채택값·대안값을 `data_quality.conflicts[]`에 기록하고 `price.confidence`는 최대 `"medium"`, 최종 결정문 검증 모드는 `DEGRADED_DATA` 대상이다. 한국 티커는 `skipped_kr` — §3의 네이버 증권·KRX 교차가 2차 소스 역할을 한다(교차 결과를 conflicts에 동일하게 기록). 비달러 쿼트 크립토(예 BTC-KRW)는 `skipped_crypto_quote` — 업비트·빗썸 교차가 같은 역할.
...
`references/data-sources.md`의 "시장 자동 감지" 규칙으로 한국/미국/크립토를 판별하고 다음을 확정한다:

- `market`: `"KR"` | `"US"` | `"CRYPTO"` — 크립토는 `{BASE}-{쿼트통화}` 표기(예 `BTC-USD`, yfinance 형식). 수집 스크립트가 자동 감지하며 `BRK-B` 같은 미국 주식 클래스 표기와 구분된다.
- `exchange`: `"KOSPI"` | `"KOSDAQ"` | `"NYSE"` | `"NASDAQ"` 등. 크립토는 `"CRYPTO(현물 종합)"` — 단일 거래소가 아니라 종합 시세임을 표기.
- `reporting_currency`: `"KRW"` | `"USD"` — 크립토는 쿼트 통화.

티커가 모호하면 웹 검색 한 번으로 상장 거래소를 확정한 뒤 진행한다. **상장 래퍼 우선 원칙:** 사용자가 "비트코인 분석"처럼 자산명만 말하면 직접 티커(BTC-USD)와 현물 ETF(IBIT 등) 중 무엇을 원하는지 오케스트레이터가 확인한다 — 실제 매매 수단이 ETF면 ETF 티커로 분석하는 쪽이 정확하다.
...
**CRYPTO 수집:** 가격·OHLCV·교차 검증은 스크립트가 처리한다(coinbase→binance). 단 **크립토는 `fast_info`의 시총·주식수가 null**이므로 시총·유통량(circulating supply)은 CoinGecko·CoinMarketCap 웹 조회로 채우고 상호 교차한다 — 유통량은 `shares_outstanding` 필드에 기록한다(시총=가격×유통량 검산이 그대로 작동). 재무 5줄·밸류에이션 배수는 재무제표가 없어 해당 없음 — `unavailable` 처리하고 `notes`에 "크립토 — 재무제표 비적용, 펀더멘털은 온체인·수급 대체"를 명시한다.
...
- **세션별 가격 서술 문구(하류 보고서 표현 규칙):** `pre_open`="전일 종가 기준" / `regular`="장중 지연 가능 시세 — 종가 아님" / `post_close`="당일 종가(확정)" / `weekend·휴장`="최근 거래일({data_as_of}) 종가 기준" / `always_open`(CRYPTO)="UTC {직전 마감일} 일봉 마감가 기준 — 당일 진행 봉({data_as_of})은 미확정, 24/7 변동 중". 단 `regular`인데 `intraday_data_gap: true`면 "장중" 서술 금지 — "휴장 또는 피드 지연 의심, 최근 거래일({data_as_of}) 종가 기준"으로 쓴다. `last_close_is_final: null`(미판정 — `anomaly` 동반)이면 데이터·판정 시각 정합을 확인하기 전까지 가격 서술 자체를 하지 않는다. "오늘/현재" 수식어는 `session.market_session`과 일치할 때만 사용. 또한 `last_close_is_final: false`면 기술지표 전체(SMA/RSI/MACD 등)가 미확정 봉을 포함해 산출된 값이다 — 하류 기술적 분석 보고서는 이를 명시한다.
- `market: "US"`인 경우에도 `fx.USDKRW`를 채워 한국 투자자 원화 환산 기준을 제공한다(CRYPTO도 동일).
- `shares_outstanding.currency`는 통화 없는 값이므로 `null`. **CRYPTO는 이 필드에 유통량(circulating supply)을 기록**한다(source에 CoinGecko/CMC 명시).
- `benchmark.primary.name`: 한국=KOSPI/KOSDAQ, 미국=S&P500(또는 분석 대상 섹터 ETF). **CRYPTO: BTC 분석이면 S&P500(거시 상관 참조), 알트코인이면 BTC-USD**(크립토 내 상대 강도가 판단 축).
