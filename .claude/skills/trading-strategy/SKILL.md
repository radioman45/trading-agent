...
description: 투자전략 트레이딩 하네스 오케스트레이터(TradingAgents 구조 + 뉴로퓨전/월가아재 톱다운 보강). 거시 전략가가 시장 레짐(risk-on/off·유동성·바벨)을 먼저 판정하고, 시장 데이터 엔지니어가 스냅샷·기술지표로 고정하며, 분석가 4명(기본적·기술적·뉴스·심리)이 병렬 분석, Bull/Bear 리서처가 2라운드 토론, 리서치 매니저가 판정해 투자 계획 수립, 트레이더가 거래 계획(진입·손절·비중·목표) 변환, 리스크 토론자 3성향(공격/중립/보수)+포트폴리오 리스크 분석가가 병렬 평가, 포트폴리오 매니저가 거시·바벨·역발상·포트폴리오 게이트로 최종 APPROVE/CONDITIONAL_APPROVE/REVISE/REJECT 판정 후 의사결정 저널에 기록하는 파이프라인을 조율한다. "트레이딩 분석", "투자전략 분석", "{티커} 매매 판단", "사야 돼 팔아야 돼", "트레이딩 전략 세워줘", "거래해도 될까", "포지션 잡아도 될까", "투자 분석", "거시 레짐", "시장 국면" 요청 시 반드시 이 스킬을 사용한다. 크립토 자산도 지원한다: "비트코인/이더리움 분석", "BTC 매매 판단", "코인 사야 돼", "크립토 트레이딩" (직접 티커 BTC-USD 또는 현물 ETF — 매매 수단을 먼저 확인). 능동 모드도 처리한다: "아이디어 스캔", "종목 발굴", "뭐 살까", "살 만한 종목", "워치리스트", "포지션 점검", "트리거 점검", "보유 등록", "포트폴리오 등록", "체결 반영", "실제로 샀어", "포트폴리오 점검", "포트폴리오 진단", "내 포트폴리오 봐줘", "리밸런싱", "포트폴리오 개선", "전체 비중 조정". 후속 요청도 처리한다: "다시 실행", "재실행", "업데이트", "거시만 다시", "레짐 갱신", "분석가 보고서만 다시", "토론 한 라운드 더", "강세론/약세론 보강", "판정만 다시", "거래계획만 다시", "리스크 검토만 다시", "최종 결정만 다시", "이전 결과 기반으로", "결과 개선", "복기", "결과 반영". 단, 단순 주가 조회나 일반 투자 상식 질문은 직접 응답해도 된다.
---
...
[점검(별도 진입)] portfolio-risk-analyst(독립 점검) → portfolio-manager(모드 3) ── 북 진단·리밸런싱 권고 ("포트폴리오 점검" 요청 시)
...
| "포트폴리오 점검/진단/개선"/"내 포트폴리오 봐줘"/"리밸런싱"/"전체 비중 조정" | **포트폴리오 점검 모드** | Phase Q로 — 보유 북 전체의 리스크 구조 진단 + 리밸런싱 권고 (Phase M과 다름 — M은 열린 결정의 트리거, Q는 계좌 단위 북 전체. 단일 종목 비중 질문은 풀 파이프라인) |
...
2. **Pending 결산 점검 (자동):** `decisions/journal.md`에 `결과: (미기록` 항목이 있으면 본 파이프라인 진행 전에 결산한다. 티커 없는 `포트폴리오 점검` 항목(Phase Q 산출)은 결산 대상이 아니다 — 건너뛴다(해당 항목은 규약상 `결과:`·`실행:` 줄 자체가 없는 것이 정상이며, 있더라도 결산하지 않는다). 항목별 티커로 수집 스크립트를 실행(`--out-dir _workspace/_pending`)해 결정 시점 대비 등락률을 구하고, 같은 기간 벤치마크 등락(당시 결정의 스냅샷/reports에 기록된 `benchmark.primary` 레벨 대조, 또는 벤치마크 티커로 스크립트 실행)으로 **알파**를 산출해 해당 항목의 `결과:` 줄을 갱신한다 — 형식: `결과: [중간점검 {YYYY-MM-DD}] 결정 후 {±X}%, 벤치마크 {±Y}% (알파 {±Z}%p) — 복기 미실시`. 이 줄 외에는 수정 금지. 갱신 내용을 사용자에게 1줄씩 보고하고 복기(Phase R)를 제안한다 — 특히 이번 분석 대상과 같은 티커면 교훈이 이번 실행에 반영되도록 복기를 먼저 권한다. 교훈 추출과 결과 확정은 복기에서만 한다.
...
1. `decisions/journal.md`에서 열린 결정(결과 미확정 + 실행 기록 있는 항목 우선)의 **무효화 조건·철회 트리거**를 수집한다. 티커 없는 `포트폴리오 점검` 항목은 제외한다(결정 단위가 아님 — 그 항목의 재점검 트리거는 Phase Q 재실행 사유일 뿐이다).
...
- **"보유 등록/갱신":** 사용자와 짧은 인터뷰로 `decisions/portfolio.json`을 작성/갱신한다(스키마: portfolio-risk 스킬). **주수(shares)·평단을 우선 수집**하고(Phase Q 시세 재평가의 실제 입력 — 주수 × 현재가), 등록 시점 `valuation` 블록(총평가액·환율·출처)을 함께 기록한다. 사용자가 주수를 모르면 비중/평가액으로 대체 가능하되 "해당 종목은 Phase Q 재평가가 제한됨(PRICE_STALE)"을 안내한다. 비중 합 검산 후 저장·커밋. 등록/갱신 종목에 열린 journal 항목(`실행:` 미체결)이 있으면 `실행:` 줄도 함께 갱신한다 — 저널은 미체결인데 보유만 등록되면 복기·pending 결산이 미진입(가상 채점)으로 오채점한다(이중 기록원 정합).
...
## Phase Q: 포트폴리오 점검 모드 (별도 진입 — 북 진단·리밸런싱 권고)

"포트폴리오 점검/진단/개선"·"리밸런싱" 요청 시. **Phase M과 다르다** — M은 열린 결정의 무효화 트리거 모니터링(결정 단위), Q는 보유 북 전체의 리스크 구조 진단(계좌 단위). 신규 거래 앵커 없이 돈다.

1. **보유 SSOT 확인:** `decisions/portfolio.json`을 읽는다. 없거나 holdings가 비면 Phase P 등록 인터뷰를 먼저 안내하고 중단한다(추측 진단 금지). `as_of` 90일 초과면 갱신을 권하되, 거부 시 `PORTFOLIO_STALE` 라벨로 진행한다.
2. **시세 재평가 + 입력 고정:** 보유 티커별로 수집 스크립트를 실행해 현재가·지표를 확보한다(`python .claude/skills/market-snapshot/scripts/collect_market_data.py --ticker {티커} --out-dir _workspace/_portfolio/{티커}` — US/KR/CRYPTO 자동, 한국 신형 영숫자 코드(0167A0류)도 자동 감지. 그래도 시장 오인이 의심되면 `.KS`(→`.KQ`) 접미사 + `--market KR`로 재시도). **시세 폴백 우선순위:** ①스크립트 → ②웹 시세(출처 태그) → ③등록 시점 평가 + `PRICE_STALE` 태그(종목 단위 — 북 라벨 `PORTFOLIO_STALE`과 별개). 환율은 웹 시세(야후 `KRW=X` 등)로 확보해 출처 태그. 현금은 등록 시점 절대액(`valuation.total_value_krw × cash_pct`)으로 고정하고 재평가 총액 대비 재정규화한다. 현재가·환율로 평가액·현재 비중을 재계산해 `_workspace/10_portfolio_input.md`를 작성한다 — 보유표(등록 비중 vs 현재 비중), 현금 비중, journal의 열린 결정 요약(채택 계획 비중·트리거·`실행:` 줄).
3. **거시 레짐 확보:** `_workspace/00_macro_regime.md`가 7일 이내(기준일 본문 확인)면 재사용, 아니면 macro-strategist(opus)를 Phase 1과 동일 프롬프트로 호출한다. 재실패 시 레짐 없이 진행(진단의 바벨 정합 축에 "레짐 미확보 — 강등" 명시).
4. **북 진단(측정):** `Agent(subagent_type="portfolio-risk-analyst", model="opus")` — 독립 점검 모드(portfolio-risk 스킬의 "독립 점검 모드" 절)로 `_workspace/10_portfolio_scan.md` 작성: 북 스냅샷·RC·집중·바벨 vs 레짐·통화·동시 꼬리 + **계획-실행 드리프트**(journal 대조) + **저널 외 보유** 표면화. 프롬프트에 명시: "각 종목 변동성은 `_workspace/_portfolio/{티커}/00_indicators.json`·`00_ohlcv_daily.csv`(2번에서 수집)에서 실측 산출하고, 수집 실패 종목만 스킬 §1의 자산군 대표치 `[추정]`을 쓴다."
5. **리밸런싱 판정:** `Agent(subagent_type="portfolio-manager", model="opus")` — 모드 3으로 `_workspace/10_portfolio_review.md` 작성 + `decisions/journal.md`에 "포트폴리오 점검" 항목 append. **개별 종목의 신규 매수/매도 확정 판단은 내리지 않는다** — 축소·증액·신규 편입이 필요한 종목은 풀 파이프라인(Phase 1~7) 회부 목록으로 권고한다(종목 판단은 토론·게이트를 거친다는 하네스 원칙). 단 명백한 규율 위반(계획 상한 초과 보유·저널 외 대형 보유·꼬리 🚩 방치)은 즉시 지적한다.
6. **정리·보고:** `reports/포트폴리오점검_{날짜}/`에 10_portfolio_scan·10_portfolio_review 복사, `git add decisions/ && git commit -m "포트폴리오 점검: {날짜}"`. 사용자 보고는 요약 박스(모드 라벨·최대 집중·동시 꼬리·바벨 정합) + 권고 우선순위 + "개별 종목 실행은 풀 파이프라인으로" 안내.

에러: 시세 확보 실패 종목은 위 폴백 우선순위(②웹 → ③등록 시점 평가+`PRICE_STALE`)를 따른다. **`shares` 부재 종목**(비중만 등록된 경우)은 재평가 자체가 불가 — 등록 비중을 그대로 쓰고 `PRICE_STALE` 태그를 단다. `PRICE_STALE` 종목의 비중 합이 북의 30%를 넘으면 북 라벨을 `PORTFOLIO_STALE`로 강등하고 재등록(주수 보강)을 권고한다. journal이 없으면 드리프트 축 생략을 진단서에 명시.
...
| 점검 입력(Q) | 오케스트레이터(포트폴리오 점검 모드) | `10_portfolio_input.md` | portfolio-risk-analyst, portfolio-manager |
| 점검 진단(Q) | portfolio-risk-analyst(독립 점검 모드) | `10_portfolio_scan.md` | portfolio-manager(모드 3) |
| 점검 판정(Q) | portfolio-manager(모드 3) | `10_portfolio_review.md`, `decisions/journal.md`(점검 항목) | 사용자, (회부 시) 풀 파이프라인 |
...
**포트폴리오 점검 흐름(Phase Q):** "내 포트폴리오 점검해줘" → portfolio.json 확인(등록 있음, as_of 신선) → 티커별 시세 재평가 → 10_portfolio_input.md → 레짐 재사용(7일 이내) → 진단: 단일 종목 47% 🚩·AI 캐펙스 노출 67% 🚩·계획 상한 초과 보유 1건 드리프트·저널 외 보유 2건 → 판정: 권고 우선순위(①집중 종목 축소 검토 회부 ②드리프트 종목 재분석 회부 ③저널 외 보유 등록·분석 권고) + journal 점검 항목 append → reports 복사·커밋. **에러 흐름:** portfolio.json 비어 있음 → "보유 등록 인터뷰 먼저" 안내 후 중단(추측 진단 금지).
