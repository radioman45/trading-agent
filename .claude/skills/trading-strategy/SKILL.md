...
## Phase 1.6: 사실 검증 게이트 (L0·L1 — 팬아웃 전 필수) [HARD]
...
```
- **PASS(⛔ 0건):** Phase 2로 진행.
- **⛔ 발견:** 해당 소스 에이전트(macro-strategist 또는 market-data-engineer)를 **1회 재호출**해 결함만 수정 → 재검증. 재실패 시 해당 수치를 "미확인"으로 강등하고 하류·최종 보고에 명시(파이프라인은 계속). 재검증은 1회로 제한(무한 루프 방지).
...
```
Agent(subagent_type="portfolio-manager", prompt="리스크 3편·05_portfolio_impact.md·04_trade_plan.md·03_research_plan.md·스냅샷·00_macro_regime.md(있으면)·00_input.md(+있으면 decisions/journal.md·lessons.md·portfolio.json)를 읽고 risk-gate 스킬 Part B 게이트로 _workspace/06_final_decision.md 작성(상단 실행 카드 5줄 의무) + decisions/journal.md에 1건 append")
```
